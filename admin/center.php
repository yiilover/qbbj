...
if($job=="config"&&$Apower[center_config])
{
	...
	$webdb[friendlink_open]?$friendlink_open1='checked':$friendlink_open0='checked';
...
elseif ($job=='blog' && $Apower[center_config])
{
	require(dirname(__FILE__)."/"."head.php");
	require(dirname(__FILE__)."/"."template/center/menu.htm");
	require(dirname(__FILE__)."/"."template/center/blog.htm");
	require(dirname(__FILE__)."/"."foot.php");
}
...
elseif($job=="user_reg"&&($Apower[user_reg]||$Apower[center_config]))
{
	...
	$YzImg_voice[intval($webdb[YzImg_voice])]=' checked ';
...
elseif($action=="config"&&($Apower[center_config]||$Apower[center_cache]||$Apower[user_reg]))
{
	...
	if(isset($webdbs[MakeIndexHtmlTime])&&!$webdbs[MakeIndexHtmlTime]&&$webdb[MakeIndexHtmlTime])
	{
		...
		rename(ROOT_PATH."$rs[htmlname]",ROOT_PATH."index.htm.bak");
	}
