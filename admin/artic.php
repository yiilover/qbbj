...
elseif($action=="work"&&$Apower[artic_listartic])
{
	...
	foreach($listdb AS $key=>$value){
		...
		if($webdb[NewsMakeHtml]==1&&$jobs=='delete'){	//删除信息后,就读不到内容了
		...
		if($webdb[NewsMakeHtml]==1&&$jobs!='delete'){	//移动栏目后,要以移动后的为标准
			$erp=get_id_table($value);
			$rs=$db->get_one("SELECT fid FROM {$pre}article$erp WHERE aid='$value'");
			$fid_str.="&bfid_array[]=$rs[fid]";
		}
...
elseif($action=="art_list"&&$Apower[artic_listartic])
{
	foreach( $list_db AS $key=>$value){
		$db->query("UPDATE {$pre}article SET list='$value' WHERE aid='$key'");
	}
