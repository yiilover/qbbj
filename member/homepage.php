...
//我的热门文章
$myhotarticleDB='';
$query = $db->query("SELECT * FROM {$pre}article WHERE mid=0 AND uid='$uid' ORDER BY hits DESC LIMIT 10");
while($rs = $db->fetch_array($query)){
	$myhotarticleDB[]=$rs;
}
...
//我的评论
$mycommentDB='';
$query = $db->query("SELECT * FROM {$pre}comment WHERE uid='$uid' ORDER BY cid DESC LIMIT 10");
while($rs = $db->fetch_array($query)){
	$mycommentDB[]=$rs;
}
