...
$Cache_FileName=ROOT_PATH."cache/fulist_cache/{$fid}_{$page}.php";
if(!$jobs&&$webdb[fulist_cache_time]&&(time()-filemtime($Cache_FileName))<($webdb[fulist_cache_time]*60)){
	echo read_file($Cache_FileName);
	exit;
}
@include_once(ROOT_PATH."data/fu_all_fid.php");
@include(ROOT_PATH."data/fu_guide_fid.php");	//栏目导航
...
function fuListMoreSort(){
	...
	while($rs=$db->fetch_array($query)){
		...
		if(!$rs[article])
		{
			$array_fid=Get_SonFid("{$pre}fu_sort",$rs[fid]);
			if($array_fid)
			{
