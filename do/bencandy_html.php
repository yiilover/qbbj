...
if(!is_writable(ROOT_PATH."cache/htm_cache/{$cacheid}_makeShow1.php"))
{
	showerr("/cache/htm_cache/{$cacheid}_makeShow1.php文件不存在,或文件不可写");
}
...
require_once(ROOT_PATH."cache/htm_cache/{$cacheid}_makeShow1.php");
...
if($fid=$fiddb[$III])
{
	...
	write_file(ROOT_PATH."cache/htm_cache/{$cacheid}_makeShow_record.php","?III=$III&fid=$fid&cacheid=$cacheid");
...
{
	unlink(ROOT_PATH."cache/htm_cache/{$cacheid}_makeShow1.php");
	//非批量生成静态,不需要看进度状况
	if($JumpUrl){
		header("location:$JumpUrl");exit;
	}
	unlink(ROOT_PATH."cache/htm_cache/{$cacheid}_makeShow_record.php");
	echo '<meta http-equiv="Content-Type" content="text/html; charset=gb2312" />';
